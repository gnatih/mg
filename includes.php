...
if (! function_exists('mix')) {
  function mix($path)
  {
    ...
    if (! $manifestFile) {
      return get_stylesheet_directory_uri().'/'.$pathWithOutSlash;
    }

    $manifestArray = json_decode(file_get_contents($manifestFile), true);

    if (file_exists(get_stylesheet_directory_uri().'/hot')) {
      return 'http://localhost:8080/'.$pathWithOutSlash;
    }

    if (array_key_exists($pathWithSlash, $manifestArray)) {
      return get_stylesheet_directory_uri().'/'.ltrim($manifestArray[$pathWithSlash], '/');
    }
